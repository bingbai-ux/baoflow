...
interface SalesChatMessagesProps {
  roomId: string
  currentUserId: string
  initialMessages: Message[]
}

export function SalesChatMessages({ roomId, currentUserId, initialMessages }: SalesChatMessagesProps) {
                        ...
                        <div className={`max-w-[70%] ${isOwn ? 'order-2' : 'order-1'}`}>
                          {!isOwn && (
                            <p className="text-[10px] text-[#888] font-body mb-1 ml-3">
                              {message.user?.display_name || (isClient ? 'クライアント' : '営業')}
                              {isClient && (
