...
export function DocumentModal({ dealId, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#EFEFEA] rounded-[16px] max-w-6xl w-full h-[90vh] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-[rgba(53,30,40,0.06)] bg-white no-print">
          <div className="flex items-center gap-2">
            <h2 className="font-display text-[14px] font-semibold text-[#351E28]">帳票発行</h2>
            {bundle && (
              <p className="text-[11px] text-[#84787D] tabular-nums">
          ...
          <button
            onClick={onClose}
            className="p-1.5 text-[#351E28] hover:bg-[#EFEFEA] rounded-[8px]"
            aria-label="閉じる"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {error ? (
            <div className="bg-[#FFD8C2] border border-[#FF5C34] rounded-[12px] p-3 text-[12px] text-[#B03616]">
              {error}
            </div>
          ) : !bundle ? (
