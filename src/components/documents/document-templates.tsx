...
function PricedTemplate({ type, deal, specs, products, variants, quotes, fees, company, banks, meta }: TemplateProps) {
  ...
  return (
    <article className="document-page bg-white text-[#351E28] print:shadow-none shadow-md">
      <header className="flex items-start justify-between border-b-2 border-[#351E28] pb-4 mb-6">
        <div>
          <h1 className="text-[32px] font-display font-semibold tracking-tight">{TYPE_TITLE[type]}</h1>
          <p className="text-[10px] text-[#84787D] font-body uppercase tracking-[0.08em] mt-1">{TYPE_TITLE_SUB[type]}</p>
          <p className="text-[11px] text-[#351E28] mt-2 tabular-nums">No. {meta.documentNumber}</p>
        </div>
        <div className="text-right text-[11px] text-[#351E28]">
          <p>発行日: <span className="tabular-nums text-[#351E28]">{today}</span></p>
          {type === 'invoice' && meta.paymentDueDate && (
            <p className="mt-1">お支払期日: <span className="tabular-nums text-[#351E28]">{formatDate(meta.paymentDueDate)}</span></p>
          )}
          {type === 'delivery_note' && meta.shippingDate && (
            <p className="mt-1">出荷日: <span className="tabular-nums text-[#351E28]">{formatDate(meta.shippingDate)}</span></p>
          )}
        </div>
      </header>
      ...
      <section className="grid grid-cols-2 gap-8 mb-6">
        <div>
          <p className="text-[14px] font-body font-semibold border-b border-[#E2E1DA] pb-1 mb-2">
            {deal.client_name_text || '(クライアント未設定)'} 御中
          </p>
          <p className="text-[12px] text-[#351E28]">案件: {deal.deal_name || '(案件名未設定)'}</p>
          <p className="text-[11px] text-[#84787D] tabular-nums">案件番号: {deal.deal_code}</p>
          {deal.desired_delivery_date && (
            <p className="text-[11px] text-[#84787D]">希望納期: {formatDate(deal.desired_delivery_date)}</p>
          )}
          {type === 'delivery_note' && (meta.shippingAddress || '') && (
            <div className="mt-3 pt-2 border-t border-[#EFEFEA]">
              <p className="text-[11px] text-[#84787D]">納品先</p>
        ...
        <div className="text-right">
          ...
          {company?.name_en && <p className="text-[11px] text-[#351E28]">{company.name_en}</p>}
          {company?.address && <p className="text-[11px] text-[#351E28] whitespace-pre-line mt-1">{company.address}</p>}
          {company?.phone && <p className="text-[11px] text-[#351E28] mt-1">TEL: {company.phone}</p>}
          {company?.email && <p className="text-[11px] text-[#351E28]">{company.email}</p>}
          {company?.registration_number && (
            <p className="text-[10px] text-[#84787D] mt-1">登録番号: {company.registration_number}</p>
          )}
          {company?.representative && <p className="text-[11px] text-[#351E28] mt-1">代表者: {company.representative}</p>}
        </div>
      </section>

      <section className="mb-6">
        <p className="text-[10px] uppercase tracking-wider text-[#84787D] mb-2">明細</p>
        <table className="w-full text-[11px] font-body border-collapse">
          <thead>
            <tr className="border-b-2 border-[#351E28] text-[#351E28]">
          ...
          <tbody>
            {lineItems.map((q, idx) => {
              ...
              return (
                <tr key={q.id} className="border-b border-[#E2E1DA] align-top">
                  <td className="py-2 pr-2 tabular-nums text-[#84787D]">{idx + 1}</td>
                  <td className="py-2 pr-2">{desc.name}</td>
                  <td className="py-2 pr-2 text-[10px] text-[#351E28]">{desc.spec}</td>
            ...
            {feeGroups.map((g, idx) => {
              ...
              return (
                <tr key={g.fee_type} className="border-b border-[#E2E1DA]">
                  <td className="py-2 pr-2 tabular-nums text-[#84787D]">{lineItems.length + idx + 1}</td>
                  <td className="py-2 pr-2">
                    {FEE_TYPE_LABELS[g.fee_type]}
                    {g.initialOnly && <span className="text-[10px] text-[#84787D] ml-1">(初回のみ)</span>}
                  </td>
                  <td className="py-2 pr-2 text-[10px] text-[#351E28]">
                    {targetSummary && <span>{targetSummary}</span>}
                    {g.notes.length > 0 && (
                      <span className="text-[#84787D]">
                        {targetSummary ? ' / ' : ''}{g.notes.join(' / ')}
                      </span>
                    )}
                  </td>
                  <td className="py-2 pr-2 text-right text-[#84787D]">-</td>
                  <td className="py-2 pr-2 text-right text-[#84787D]">-</td>
                  <td className="py-2 text-right tabular-nums font-semibold">
                    {formatJPY(g.total)}
                  </td>
      ...
      <section className="grid grid-cols-2 gap-6 mb-6">
        <div className="text-[10px] text-[#84787D]">
          {meta.notes && (
            <>
              <p className="uppercase tracking-wider mb-1">備考</p>
              <p className="text-[#351E28] whitespace-pre-line text-[11px]">{meta.notes}</p>
            </>
          )}
        </div>
      ...
      {type === 'invoice' && banks && banks.length > 0 && (
        <section className="border border-[#E2E1DA] rounded-[12px] p-3 mb-4">
          <p className="text-[11px] font-body font-semibold text-[#351E28] mb-2">お振込先</p>
          <div className="grid grid-cols-2 gap-3 text-[11px] text-[#351E28]">
            {banks.map((b, i) => (
              <div key={i}>
                <p className="font-semibold text-[#351E28]">{b.bank_name} {b.branch_name || ''}</p>
                <p className="tabular-nums">{b.account_type ? `${b.account_type} ` : ''}{b.account_number || ''}</p>
                {b.account_holder && <p>{b.account_holder}</p>}
                {b.swift_code && <p className="text-[10px] text-[#84787D]">SWIFT: {b.swift_code}</p>}
              </div>
            ))}
          </div>
        </section>
...
function RfqTemplate({ deal, products, variants, meta, company }: TemplateProps) {
  ...
  return (
    <article className="document-page bg-white text-[#351E28] print:shadow-none shadow-md">
      <header className="border-b-2 border-[#351E28] pb-4 mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[28px] font-display font-semibold tracking-tight">Request for Quotation</h1>
            <p className="text-[12px] text-[#351E28] font-body mt-0.5">见积询价书</p>
            <p className="text-[11px] text-[#351E28] mt-2 tabular-nums">No. {meta.documentNumber}</p>
          </div>
          <div className="text-right text-[11px] text-[#351E28]">
            <p>Date / 发出日: <span className="tabular-nums text-[#351E28]">{today}</span></p>
            {deal.desired_delivery_date && (
              <p className="mt-1">Required Delivery / 期望交期: <span className="tabular-nums text-[#351E28]">{formatDate(deal.desired_delivery_date)}</span></p>
            )}
          </div>
        </div>
      </header>

      <section className="mb-5 text-[11px] text-[#351E28] font-body">
        <p className="font-semibold mb-1">Dear Supplier / 致供应商各位</p>
        <p className="text-[#351E28] leading-relaxed">
        ...
        <p className="text-[#351E28] leading-relaxed mt-1">
      ...
      <section className="mb-5 grid grid-cols-2 gap-6 text-[11px]">
        <div>
          <p className="text-[10px] uppercase tracking-[0.08em] text-[#84787D] mb-1">From / 委托方</p>
          {company?.name ? (
            <>
              <p className="font-body font-semibold">{company.name}</p>
              {company.name_en && <p className="text-[#351E28]">{company.name_en}</p>}
              {company.email && <p className="text-[#351E28] mt-0.5">{company.email}</p>}
              {company.phone && <p className="text-[#351E28]">TEL: {company.phone}</p>}
            </>
          ) : (
            <p className="text-[#84787D]">(Please set company info)</p>
          )}
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.08em] text-[#84787D] mb-1">Project / 项目</p>
          <p className="font-body font-semibold">{deal.deal_name || '(Untitled project)'}</p>
          <p className="text-[#84787D] tabular-nums mt-0.5">Code: {deal.deal_code}</p>
        </div>
      </section>

      <section className="mb-5">
        <p className="text-[10px] uppercase tracking-wider text-[#84787D] mb-2">Items / 产品规格</p>
        {products.length === 0 ? (
          <p className="text-[11px] text-[#84787D]">No products registered.</p>
        ) : (
          <div className="space-y-3">
            {products.map((p) => {
              const vs = byProduct.get(p.id) || []
              return (
                <div key={p.id} className="border border-[#E2E1DA] rounded-[12px] p-3">
                  <p className="text-[12px] font-body font-semibold text-[#351E28]">
                    #{p.product_no} {p.description}
                  </p>
                  {vs.length === 0 ? (
                    <p className="text-[11px] text-[#84787D] mt-1">No variants / 无规格</p>
                  ) : (
                    <table className="w-full text-[10px] font-body mt-2">
                      <thead>
                        <tr className="border-b border-[#E2E1DA] text-[#351E28]">
                      ...
                      <tbody>
                        {vs.map((v) => (
                          <tr key={v.id} className="border-b border-[#EFEFEA] align-top">
                            <td className="py-1.5 pr-2 font-semibold tabular-nums">{v.variant_label}</td>
                            <td className="py-1.5 pr-2 tabular-nums text-[#351E28]">
                              {v.width_mm && v.height_mm ? `${v.width_mm}×${v.height_mm}${v.depth_mm ? `×${v.depth_mm}` : ''}` : '-'}
                            </td>
                            <td className="py-1.5 pr-2 text-[#351E28]">{v.material || '-'}</td>
                            <td className="py-1.5 pr-2 text-[#351E28]">
                              {[v.print_method, v.print_color_count].filter(Boolean).join(' ') || '-'}
                            </td>
                            <td className="py-1.5 text-[#351E28] tabular-nums">
      ...
      <section className="mb-5 border border-[#351E28] rounded-[12px] p-3 text-[11px]">
        <p className="font-body font-semibold text-[#351E28] mb-2">Please provide / 请提供</p>
        <ol className="list-decimal list-inside space-y-1 text-[#351E28]">
      ...
      {meta.notes && (
        <section className="mb-5 text-[11px]">
          <p className="text-[10px] uppercase tracking-wider text-[#84787D] mb-1">Notes / 备注</p>
          <p className="text-[#351E28] whitespace-pre-line">{meta.notes}</p>
        </section>
      )}

      <section className="mb-5 text-[11px] text-[#351E28]">
...
function Row({
  ...
  return (
    <div className="flex items-baseline justify-between gap-3">
      <dt className="text-[#351E28]">{label}</dt>
      <dd
        className={`tabular-nums ${large ? 'text-[18px] font-semibold' : 'text-[12px]'} ${
          accent ? 'text-[#666C14]' : 'text-[#351E28]'
        }`}
      >
        {value}
      </dd>
