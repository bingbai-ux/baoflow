    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {factory.name_cn && (
            <p className="text-[11px] text-[#351E28] font-body mt-0.5">{factory.name_cn}</p>
          )}
          <p className="text-[10px] text-[#84787D] mt-0.5">
        ...
        <Section title="得意分野">
          <div className="flex flex-wrap gap-1">
            {factory.specialties.map((s) => (
              <span key={s} className="inline-flex items-center text-[10px] bg-[#EFEFEA] text-[#351E28] px-2 py-0.5 rounded-full">
                {s}
              </span>
            ))}
          </div>
      ...
      {factory.notes && (
        <Section title="メモ">
          <p className="text-[11px] text-[#351E28] whitespace-pre-line">{factory.notes}</p>
        </Section>
      )}
    </div>
  )
...
function Stat({ label, value, unit, accent }: { label: string; value: string; unit?: string; accent?: boolean }) {
  return (
    <div className="bg-[#FBFAF6] rounded-[12px] p-2.5">
      ...
      <p className={`font-display tabular-nums leading-none mt-1 ${accent ? 'text-[#666C14] text-[18px]' : 'text-[#351E28] text-[18px]'}`}>
    ...
    <div className="bg-[#FBFAF6] rounded-[12px] p-2.5">
      ...
      <div className="flex items-center gap-0.5 mt-1">
        {[1, 2, 3, 4, 5].map((n) => (
          <Star
            key={n}
            className={`w-3 h-3 ${value && n <= value ? 'text-[#B03616] fill-[#FF5C34]' : 'text-[#E2E1DA]'}`}
          />
        ))}
      </div>
    </div>
